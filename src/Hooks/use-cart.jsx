...
const useCart = () => {
  ...
  const cartUpdate = async (data) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/Shoesar/Users/Cart`,
        {
          method: "POST",
          headers: {
      ...
      toast.update(id, {
        render: "Error, no realice cambios durante el guardado",
        type: "error",
        isLoading: false,
        autoClose: true,
      });
  ...
  const tFail = () => {
    toast.dismiss();
    toast.error("No se encuentra disponible", {
      position: "top-center",
      autoClose: 1500,
    });
  };
  const cartReducer = (state, action) => {
    switch (action.type) {
      case "ADD_TO_CART":
        ...
        if (foundIndex !== -1) {
          if (
            state[foundIndex].Amount + action.payload.Amount <=
            action.payload.Stock
          ) {
            const updatedItem = {
              ...state[foundIndex],
              Amount: state[foundIndex].Amount + action.payload.Amount,
            };
            tSuc();
            return [
      ...
      case "DECREASE_AMOUNT":
        ...
        if (state[decreaseIndex].Amount - 1 !== 0) {
          const decreasedItem = {
            ...state[decreaseIndex],
            Amount: state[decreaseIndex].Amount - 1,
          };
          return [
            ...state.slice(0, decreaseIndex),
            decreasedItem,
            ...state.slice(decreaseIndex + 1),
          ];
        }
        return [
          ...state.slice(0, decreaseIndex),
      ...
      case "INCREASE_AMOUNT":
        ...
        if (state[increaseIndex].Amount + 1 <= state[increaseIndex].Stock) {
          const increasedItem = {
            ...state[increaseIndex],
            Amount: state[increaseIndex].Amount + 1,
          };
          return [
            ...state.slice(0, increaseIndex),
            increasedItem,
            ...state.slice(increaseIndex + 1),
          ];
        }
        tFail();
        return state;
  ...
  const update = async () => {
    if (!isCartUpdateRunning.current) {
      ...
      toast.error("Error al guardar, espere unos segundos...", {
        position: "top-center",
        autoClose: 3000,
      });
    }
