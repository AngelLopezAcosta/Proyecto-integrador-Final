  ...
  const altaReducer = (state, action) => {
    ...
    switch (action.type) {
      case "NAME":
        ...
        if (content === "") {
          ...
          if (!/^[0-9a-zA-ZáéíóúüñÁÉÍÓÚÜÑ \-'.]+$/.test(content)) {
            message.push(
              "Caracteres especiales no permitidos, solo letras, números, espacios, guiones y comillas."
            );
          }
        }
      ...
      case "PRICE":
        ...
        if (content === "") {
          ...
          if (isNaN(stockValue) || stockValue < 0) {
            message.push(
              "El precio no puede ser negativo y debe ser un número válido."
            );
          }
        }
        stateCopy = structuredClone(state);
      ...
      case "BRAND":
        ...
        message = [];
        if (content === "") {
          message.push("La marca de zapato es obligatoria.");
        } else {
          if (!/^[0-9a-zA-ZáéíóúüñÁÉÍÓÚÜÑ \-'.]+$/.test(content)) {
            message.push(
              "Caracteres especiales no permitidos, solo letras, números, espacios, guiones y comillas."
          ...
          if (content.toLowerCase().includes("marca")) {
            message.push(
              "Evita incluir la palabra 'marca' en la marca del zapato."
            );
          }
        }
        stateCopy = structuredClone(state);
      ...
      case "STOCK":
        ...
        message = [];
        if (content === "") {
          message.push("Se requiere una cantidad que esté disponible.");
        } else {
          const stockValue = parseInt(content);
          if (isNaN(stockValue) || stockValue < 0) {
            message.push(
              "La cantidad disponible no puede ser negativa y debe ser un número válido."
            );
          }
          if (stockValue !== parseFloat(content)) {
            message.push("La cantidad disponible no puede ser una fracción.");
          }
        }
        stateCopy = structuredClone(state);
      ...
      case "IMAGE":
        ...
        if (content === "") {
          ...
          if (!/^https?:\/\/[^\s/$.?#].[^\s]*$/i.test(content)) {
            message.push("Error, la URL no es válida.");
          } else {
      ...
      case "SHORT":
        ...
        if (content === "") {
          ...
          if (!/^[a-zA-ZáéíóúüñÁÉÍÓÚÜÑ0-9\s.,;:!?"'-]*$/.test(content)) {
            message.push(
              "La descripción corta contiene caracteres no válidos."
            );
          }
        }
      ...
      case "LONG":
        ...
        if (content === "") {
          ...
          if (content.length < 30) {
            message.push(
              "La descripción larga debe tener entre 30 y 200 caracteres."
            );
          }

          if (!/^[a-zA-ZáéíóúüñÁÉÍÓÚÜÑ0-9\s.,;:!?"'-]*$/.test(content)) {
            message.push(
              "La descripción larga contiene caracteres no válidos."
            );
          }
        }
        stateCopy = structuredClone(state);
