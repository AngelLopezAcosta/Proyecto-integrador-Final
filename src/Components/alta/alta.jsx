...
const Alta = () => {
  ...
  const post = usePost(
    "loading",
    "Zapato agregado exitosamente!",
    "Error al agregar zapato."
  );
  const submitHandler = (event) => {
    ...
    if (
      ...
      post(
        { newShoe: newShoe, email: userData.email, apiKey: userData.apiKey },
        `${process.env.REACT_APP_API_URL}/Shoesar/Products/Add`
      );
      ...
      <section className="Alta">
        <article className="Alta_body">
          <form className="form" id="personalInfo" onSubmit={submitHandler}>
            ...
            <Descriptions
              shortDes={alta.shortDes}
              setShortDes={setShortDes}
              longDes={alta.longDes}
              setLongDes={setLongDes}
              makeError={makeError}
            />
